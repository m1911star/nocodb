...
<template>
  <div ref="container" style="box-shadow: 0px -12px 16px -4px rgba(0, 0, 0, 0.1), 0px -4px 6px -2px rgba(0, 0, 0, 0.06)"
    :style="!isMobileMode ? 'width: min(80svw, 520px);' : ''" :class="{
    ...
    <div class="space-y-3">
      <div class="flex px-6 justify-between items-center">
        <span class="text-md font-bold text-gray-800" @click.stop> {{ $t('general.notification') }} </span>

        <NcButton v-if="isMobileMode" size="small" type="secondary" @click="emits('close')">
          <GeneralIcon icon="close" class="text-gray-700" />
        </NcButton>
      </div>
      <div v-if="notificationTab !== 'read'" :class="{
      ...
      <NcTabs v-model:activeKey="notificationTab">
        <a-tab-pane key="unread">
          ...
          <div class="overflow-y-auto" :style="`height: ${height - 72}px`" :class="{
        ...
        <a-tab-pane key="read">
          ...
          <div class="overflow-y-auto" :style="!isMobileMode ? `height: ${height - 72}px` : ''" :class="{
